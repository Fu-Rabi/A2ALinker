...
import subprocess
p = subprocess.Popen(
    'tail -f /tmp/a2a_host_in | ssh -o UserKnownHostsFile=.agents/skills/a2alinker/known_hosts -o StrictHostKeyChecking=yes -o ServerAliveInterval=30 -o ServerAliveCountMax=10 -p $PORT $TOKEN@$SERVER create',
    shell=True,
